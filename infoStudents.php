            ...
            <div>
                <h2>
                    ...
                    <?php
                        if (isset($_SESSION['email'])) { 
                            $email = $_SESSION['email']; 
                            $get = mysqli_query($conn, "select student_id from students where email = '$email' ");
                            $getData = $get->fetch_all(MYSQLI_ASSOC);
                            $mssv = $getData[0]['student_id'];
                            echo htmlspecialchars($mssv);
                        }
                    ?>
                </h2>
            </div>
            ...
            <div>
                <h2>
                    ...
                    <?php
                        if (isset($_SESSION['email'])) { 
                            $email = $_SESSION['email']; 
                            $get = mysqli_query($conn, "select date_of_birth from students where email = '$email' ");
                            $getData = $get->fetch_all(MYSQLI_ASSOC);
            ...
            <div>
                <h2>
                    ...
                    <?php
                        if (isset($_SESSION['email'])) { 
                            $email = $_SESSION['email']; 
                            $get = mysqli_query($conn, "select gender from students where email = '$email' ");
                            $getData = $get->fetch_all(MYSQLI_ASSOC);
            ...
            <div class="last-service">
                <h2>
                    ...
                    <?php
                        if (isset($_SESSION['email'])) { 
                            $email = $_SESSION['email']; 
                            $get = mysqli_query($conn, "select phone_number from students where email = '$email' ");
                            $getData = $get->fetch_all(MYSQLI_ASSOC);
